...
const projectController = (() => {
  ...
  function _createProjectInstance(title) {
    const newProjectInst = new Project(title);
    projectList.push(newProjectInst);

    storageController.updateProjectListStorage();

    return newProjectInst;
  }
  ...
  function _createProjectDOM(newProject, defaultSection) {
    const sidebarDefaultSection = document.querySelector(
  ...
  function createNewProject(title, defaultSection = false) {
    const newProjectVar = _createProjectInstance(title);
    _createProjectDOM(newProjectVar, defaultSection);
  }
  ...
  function deleteProject(projectToDelete) {
    ...
    const projects = sidebarProjectSection.querySelectorAll(
...
const taskController = (() => {
  ...
  function _completeTask(taskItem) {
    ...
    if (taskItem.previousElementSibling !== null) {
      if (
        taskItem.previousElementSibling.classList.contains("main-task-divider")
      ) {
...
const viewController = (() => {
  ...
  function _updateMainTaskView(selectedProjectDOM) {
    const currentProject = getProjectFromId(
      projectList,
      selectedProjectDOM.dataset.projectId,
    );
    ...
    switch (currentProject.id) {
      case "today":
        ...
        taskItemList.forEach((task) => {
          if (task.dueDate === format(new Date(), "yyyy-MM-dd")) {
            taskController.createNewTaskDOM(
              task.title,
              task.description,
              task.dueDate,
              task.priority,
      ...
      case "upcoming":
        ...
        taskItemList.forEach((task) => {
          if (
            isWithinInterval(new Date(parseISO(task.dueDate)), {
              start: new Date(),
  ...
  function toggleFormModalVisibility(className) {
    ...
    const { display } = window.getComputedStyle(elementToToggle);
  ...
  function setupTaskViewModal(taskItem) {
    ...
    const headerProject = document.querySelector(
    ...
    const taskDescriptionContainer = document.querySelector(
      ".task-view-modal-main-left-description",
    );
    const taskDescription =
      taskDescriptionContainer.querySelector(":scope > div");
    taskDescription.textContent =
      task.description === "" ? "Description" : task.description;

    const rightProject = document.querySelector(
    ...
    const rightDueDateContainer = document.querySelector(
    ...
    const rightPriorityContainer = document.querySelector(
...
(function initialPageSetup() {
  (function _setupEventListeners() {
    ...
    const taskViewModalCloseBtn = document.querySelector(
    ...
    newProjectModalForm.addEventListener("submit", (e) => {
      ...
      projectController.createNewProject(
        getAddProjectFormValues(newProjectModalForm),
      );
      viewController.toggleFormModalVisibility(".new-project-modal-container");
    ...
    deleteProjectBtn.addEventListener("click", () => {
      const mainProjectId =
        document.querySelector(".main-project-name").dataset.projectId;

      // Exit clause to not allow deletion of constant section projects
      if (
        mainProjectId === "general" ||
        mainProjectId === "today" ||
        mainProjectId === "upcoming"
      ) {
        return;
      }
      const projectToDelete = getProjectFromId(projectList, mainProjectId);
      const projectGeneral = document.querySelector(
        ".sidebar-default-section > div",
      );
    ...
    createTaskForm.addEventListener("submit", (e) => {
      ...
      taskController.createNewTaskInstance(
        taskData.title,
    ...
    taskViewModalCloseBtn.addEventListener("click", () => {
      viewController.toggleFormModalVisibility(".task-view-modal-container");
    });
  })();
  ...
  (function _setupProjects() {
    ...
    if (projectListStorage === null) {
      projectController.createNewProject("General", true);
      projectController.createNewProject("Today", true);
      projectController.createNewProject("Upcoming", true);
    } else {
      projectListStorage.forEach((project) => {
        if (
          ...
          projectController.createNewProject(project.title, true);
        } else {
          projectController.createNewProject(project.title, false);
        }
      });
    }
  })();
